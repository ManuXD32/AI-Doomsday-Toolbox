...
# Build the install-time Debian guest from a pinned official OCI image. The build intentionally
...
ASSET_DIR="$ROOT_DIR/src/main/assets/debian"
OCI_REF="docker.io/library/debian@sha256:7215f78f35ffe58fe13f244fac9c4f21326d55187271fbb3e1a8aa5cc7e387ab"
OCI_DIGEST="sha256:7215f78f35ffe58fe13f244fac9c4f21326d55187271fbb3e1a8aa5cc7e387ab"
OCI_CREATED="2026-08-24"
SNAPSHOT="20260824T000000Z"
IMAGE_ID="debian-trixie-arm64-20260824"
...
# The quoted heredoc is important: `${Package}` is a dpkg-query format token and must reach
# the arm64 guest unchanged rather than being interpreted by this host shell.
podman run --rm -i --arch arm64 \
  --network=host \
  -v "$build_dir:/out:Z" \
...
# Keep apt usable for deliberate user installs while discarding resolver caches and transient
# logs from the signed bootstrap image.
apt-get clean
...
# Export a deterministic tarball. Device/pseudo-filesystem contents are deliberately excluded;
# the app broker provides only the minimal /proc and device binds at execution time.
...
cat > "$ASSET_DIR/manifest.json" <<EOF_MANIFEST
{
  ...
  "packageInventory": "rootfs.manifest",
  "licenses": "licenses/",
  "notes": "Pinned arm64 Debian Trixie developer rootfs. User apt installs modify only the writable guest copy and never the signed bootstrap asset."
}
EOF_MANIFEST

echo "Prepared Debian Trixie arm64 rootfs: $ASSET_DIR/rootfs.tar.xz"
