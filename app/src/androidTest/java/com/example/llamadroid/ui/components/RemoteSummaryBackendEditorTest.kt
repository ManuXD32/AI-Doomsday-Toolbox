...
class RemoteSummaryBackendEditorTest {
    ...
    fun urlDraftSurvivesParentFallbackAndBackendSwitch() {
        composeRule.setContent {
            var backend by mutableStateOf(SettingsRepository.PDF_BACKEND_OLLAMA)
            var ollamaUrl by mutableStateOf("http://persisted:11434")
            var llamaServerUrl by mutableStateOf("http://server:8080")
            var llamaSwapUrl by mutableStateOf("http://swap:9292")
        ...
        urlField.assertTextEquals("")
        "https://demo.local/api".forEach { urlField.performTextInput(it.toString()) }
        urlField.assertTextEquals("https://demo.local/api")
        ...
        composeRule.onNodeWithTag("remote_summary_url_field").assertTextEquals("https://demo.local/api")
    }
}
